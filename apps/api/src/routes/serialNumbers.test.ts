...
describe("GET /product-numbers/:productNumber/serial-numbers", () => {
  it("lists distinct serial numbers for a product number", async () => {
    ...
    ins.run("ds2", "531285301", "S2", "snap-2", "2026-02-18T07:50:23.000Z");
    ins.run("ds3", "999", "S9", "snap-x", "2026-02-01T00:00:00.000Z");
    seed.close();
    ...
    const res = await app.inject({
      method: "GET",
      url: "/product-numbers/531285301/serial-numbers"
    });

    expect(res.statusCode).toBe(200);
    expect(res.json()).toEqual(["S1", "S2"]);
